...
xYearStat <- function(x, STATNAME){
  #generate data.table with rolling means, window=x
  frollapply(da[, .(de_bilt, eelde, maastricht)], 20, FUN = STATNAME)
  xYrm <- as.data.table(frollmean(da[, .(de_bilt, eelde, maastricht)], x))[, ID := .I ]
  keepYearsX <- seq(from = 1, to = 120, by = x) #used to drop all other rows
...
meanTable10 <- xYearMean(10)
meanTable5 <- xYearMean(5)
meanTable20 <- xYearMean(20)
meanTable50 <- xYearMean(50)

medianTable5 <- xYearStat(5, median)
